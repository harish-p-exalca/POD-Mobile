...
export class GetService{
 ...
 LeadTimeChartData(usercode:string,userid:string,role:string):Observable<any>{
  if(role=="Customer"){
   return this.http.get<any>(this.baseUrl+"/api/Dashboard/FilterLeadTimeCountByUser?UserCode="+usercode).pipe(
     retry(5),
     catchError(this.handleError<any>('getHeroes', []))
   );
  }
  else{
   return this.http.get<any>(this.baseUrl+"/api/Dashboard/FilterLeadTimeCount?UserID="+userid).pipe(
     retry(5),
     catchError(this.handleError<any>('getHeroes', []))
   );
  }
